...
## 0. 当前 880 待排期索引模式（最高优先级）

当用户在批量录入 880、目标是先建立待排期题目池并在之后每天按顺序取题时，使用本节，而不是后文的完整 OCR/排期默认流程。

### 写入范围

- 唯一目标文件是 `workbook_880_unscheduled.json`。
- 不写入或修改 `880-record.json`、`review_state.json`、`review_schedule.json`、`math_records.json`、ABC 汇总文件、进度索引及任何已排期数据。
- 卡片只记录 `page`、`question_type`、`question_id`、`display_id`、`display_label`、`error_level` 和既有的顺序字段；不记录题干、答案、解析或 LaTex。

### 图片判读

- 页码使用题目开始所在的物理页码；续页不另建重复卡，除非用户明确要求。
- 勾号或用户说“对了”代表做对，不记录。用户口头说明的等级优先于图上笔记。
- 默认一题一张卡；只有用户明确指定“第几小问”时才拆成小问卡。
- 没有等级、且图片标记无法安全辨认时，直接简短询问；不要 OCR 题干、不要解题、不要猜等级。

### 操作节奏与校验

1. 只看当前用户提供的图片和待排期文件末尾，确认将新增的页码、题型、题号与等级。
2. 一次性写入当前批次；不得为寻找旧题、答案或全文题干反复搜索。
3. 只验证 JSON 可解析、ID 唯一、写入数与待排期总数正确。
4. 回复只报本批新增数、待排期总数和确有必要的一项待确认信息。

后续用户要求“每天取一些题”时，按 `workbook_880_unscheduled.json` 的既有顺序取出；在用户明确要求前仍不排期。
